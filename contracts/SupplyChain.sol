...
contract SupplyChain {
  ...
  */
  enum State {ForSale, Sold, Shipping, Received}
...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
   ...
   */
  modifier forSale (uint sku) {require((items[sku].state == State.ForSale && bytes(items[sku].name).length != 0), 'Item state is not for sale'); _;}
  modifier sold (uint sku) {require(items[sku].state == State.Sold, 'Item state is not sold'); _;}
  modifier shipped (uint sku) {require(items[sku].state == State.Shipping, 'Item state is not shipped'); _;}
  modifier received (uint sku) {require(items[sku].state == State.Received, 'Item state is not received'); _;}
